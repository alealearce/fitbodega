...
export default async function Top100FitnessRetreatsPage() {
  const meta = data.meta as unknown as Meta;
  const entries = data.entries as unknown as LedgerEntry[];
  const claimed = await getClaimedMap("retreats");
    ...
    <main className="bg-bg">
      ...
      <section className="py-20 lg:py-24 bg-bg">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <Top100Ledger
            entries={entries}
            claim={{ list: "retreats", claimed }}
            config={{
              scoreName: meta.scoreModel.name,
              kicker: "Know before you go",
              showCity: true,
              tierChips: { "training-camp": "Camp" },
            }}
          />
        </div>
      </section>
