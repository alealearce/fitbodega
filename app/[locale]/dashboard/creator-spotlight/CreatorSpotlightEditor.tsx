...
type SpotlightProfile = Pick<
  CreatorProfile,
  "id" | "name" | "spotlight_story" | "spotlight_images" | "spotlight_opt_out" | "spotlight_post_id"
>;

export default function CreatorSpotlightEditor({ profile }: { profile: SpotlightProfile }) {
  const listing = profile;
  const [answers, setAnswers] = useState<Record<string, string>>(
    (listing.spotlight_story as Record<string, string>) ?? {}
  );
  const [optOut, setOptOut] = useState(listing.spotlight_opt_out);
  const [existing, setExisting] = useState<string[]>(listing.spotlight_images ?? []);
  ...
  const addPhotos = async (files: FileList | null) => {
    ...
    for (const raw of Array.from(files)) {
      ...
      if (file.size > MAX_IMAGE_SIZE) { setPhotoError("We couldn't process that photo — please try a smaller one."); continue; }
  ...
  const save = async () => {
    ...
    try {
      ...
      formData.set(
        "payload",
        JSON.stringify({
          spotlight_story: answers,
          spotlight_opt_out: optOut,
          existing_images: existing,
        })
      );
      newPhotos.forEach((p, i) => formData.set(`spotlightImage${i}`, p.file));

      const res = await fetch("/api/creators/spotlight", { method: "POST", body: formData });
      const data = await res.json();
      if (res.ok) {
        setStatus("saved");
        if (Array.isArray(data.spotlight_images)) {
          setExisting(data.spotlight_images);
      ...
      <div className="bg-surface-low p-8 space-y-6">
        ...
        >
          ...
          <span className="font-sans text-sm text-on-surface-variant group-hover:text-on-surface transition-colors">
            Prefer not to be featured? Check this and we simply keep your profile in the network — no spotlight,
            no social feature.
          </span>
        </button>
          ...
          <>
            ...
            <div>
              <p className="font-sans text-sm text-on-surface-variant mb-4">
                Up to {MAX_STORY_PHOTOS} photos of you training or making your content — a portrait works best
